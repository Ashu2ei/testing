...
from authentication.models import User , Input , Stutus
...
class Checking(APIView):
    def post(self, request, format = None):
        try:
            data = request.data
            obj = Stutus.objects.create(text = data['text'])
            for data in data['many']:
                try:
                    many_obj = Input.objects.get(name = data)
                    obj.choices.add(many_obj)
                except:
                    pass
            return Response({'status':'SUCCESS'},status=200)
        except Exception as e:
